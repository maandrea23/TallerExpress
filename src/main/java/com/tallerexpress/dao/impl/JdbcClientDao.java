...
import com.tallerexpress.dao.ClientDao;
...
public final class JdbcClientDao implements ClientDao {
    public Client create(Connection c, Client client) {
        try (PreparedStatement q = c.prepareStatement(
            ...
            q.setString(1, client.document());
            q.setString(2, client.name());
            q.setString(3, client.phone());
            q.setString(4, client.email());
            q.executeUpdate();
            try (ResultSet r = q.getGeneratedKeys()) {
                r.next();
                return new Client(r.getLong(1), client.document(),
                        client.name(), client.phone(), client.email(), true, client.createdAt());
            }
        } catch (SQLException e) {
            throw error("Error creando el cliente", e);
        }
    }
    ...
    public List<Client> findAll(Connection c) {
        ...
        try (Statement q = c.createStatement(); ResultSet r = q.executeQuery("SELECT * FROM clients ORDER BY id")) {
            ...
            throw error("Error consultando clientes", e);
        }
    }
    ...
    public boolean isActive(Connection c, long id) {
        try (PreparedStatement q = c.prepareStatement("SELECT active FROM clients WHERE id=?")) {
            ...
            throw error("Error consultando el cliente", e);
        }
    }

    private DataAccessException error(String message, SQLException e) {
